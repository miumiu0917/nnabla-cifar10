...
class InputData(object):
  ...
  def __init__(self, data_dir='./data'
    ...
    self.idx = 0
    self.test_labels, self.test_images = self.__test_data_batches(test_data) 
  ...
  def __test_data_batches(self, test_data):
    labels = []
    images = []
    idx = 0
    while True:
      if idx + self.batch_size > len(test_data):
        return labels, images
      batch = test_data[idx:idx+self.batch_size]
      labels.append([self.__one_hot_vector(label[0]) for (label, image) in batch])
      images.append([image for (label, image) in batch])
      idx += self.batch_size
  ...
  def test_data(self):
    for (test_label, test_image) in zip(self.test_labels, self.test_images):
      yield test_label, test_image
